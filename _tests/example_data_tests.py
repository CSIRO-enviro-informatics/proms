import os
import requests
import _database  # must have PROMS Server & DB running

PROMS_BASE_URI = 'http://localhost:9000'
PROMS_HOME_DIR = 'c:/work/proms/'
...
def load_rs():
    ...
    r = requests.post(
        ...
        data=open(os.path.join(PROMS_HOME_DIR, '_tests', 'example_data_rs_01.ttl')).read(),
...
def load_reports():
    ...
    r = requests.post(
        ...
        data=open(os.path.join(PROMS_HOME_DIR, '_tests', 'example_data_rs_01_report_01.ttl')).read(),
        headers={'Content-Type': 'text/turtle'}
    )
    print(r.content)
    assert r.status_code == 201
    ...
    r = requests.post(
        PROMS_BASE_URI + '/function/lodge-report',
        data=open(os.path.join(PROMS_HOME_DIR, '_tests', 'example_data_rs_01_report_02.ttl')).read(),
        headers={'Content-Type': 'text/turtle'}
    )
    print(r.content)
    assert r.status_code == 201
    ...
    r = requests.post(
        PROMS_BASE_URI + '/function/lodge-report',
        data=open(os.path.join(PROMS_HOME_DIR, '_tests', 'example_data_rs_01_report_03.ttl')).read(),
        headers={'Content-Type': 'text/turtle'}
    )
    print(r.content)
    assert r.status_code == 201
    ...
    r = requests.post(
        PROMS_BASE_URI + '/function/lodge-report',
        data=open(os.path.join(PROMS_HOME_DIR, '_tests', 'example_data_rs_01_report_04.ttl')).read(),
        headers={'Content-Type': 'text/turtle'}
    )
    print(r.content)
    assert r.status_code == 201
