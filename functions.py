...
from proms_rulesets.rulesets.reportingsystems import ReportingSytems
...
def get_reportingsystem_dict(reportingsystem_uri):
    ...
    '''
    reportingsystem_detail = functions_db.query(query)
    ret = {}
...
def get_reports_for_rs(reportingsystem_uri):
    ...
    return functions_db.query(query)
...
def put_reportingsystem(reportingsystem_in_turtle):
    ...
    # Validation
    conf_results = ReportingSytems(g).get_result()
    fail_reasons = []
    ...
    if len(fail_reasons) == 0:
        result = functions_db.insert_secure(reportingsystem_in_turtle, True)
        if result[0]:
            query = '''
                PREFIX proms: <http://promsns.org/def/proms#>
                SELECT  ?r
...
def get_report_rdf(report_uri):
    ...
    '''
    return functions_db.query_turtle(query)
...
def register_pingback(data):
    ...
    for row in query_result:
        if len(row) == 2:
            ...
            '''
            db_result = functions_db.insert_secure(query, True)
            if not db_result[0]:
...
def get_entity_activity_wgb_svg(entity_uri):
    ...
    '''
    entity_results = functions_db.query(query)
...
WHERE {
    ...
    '''
    entity_result = functions_db.query(query)
...
WHERE {
    ...
    '''
    activity_results = functions_db.query(query)
...
def get_sparql_service_description(format='turtle'):
    """Return an RDF description of PROMS' read only SPARQL endpoint in a requested format

    :param format: 'turtle', 'n3', 'xml', 'json-ld'
