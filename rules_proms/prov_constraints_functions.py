...
def checkCycle(g):
	...
	SELECT ?x
	WHERE {
		?x (c:precedes+|c:strictlyPrecedes+)/c:strictlyPrecedes ?x .
	}
	'''

	g1 = orderingConstraints(g)

	if not check(g1, qCheckCycle):
		return 'qCheckCycle'
	return None
...
def checkImpossibility(g):
	queries = ['impossible_unspecified_derivation_generation_use',
			   'impossible_specialization_reflexive',
...
def check(g, q):
	...
	:param q: query
	:return: success of query against graph
	"""
	bindings = g.query(q)
	if len(bindings) > 0:
		# for b in bindings:
...
unique_wasStartedBy = '''
	...
	SELECT ?a WHERE {
		...
		?start1 prov:hadActivity ?starter1 .
		?start2 prov:hadActivity ?starter2 .
		FILTER ( ?starter1 != ?starter2)
	}
'''
...
unique_wasEndedBy = '''
	...
	SELECT ?a WHERE {
		...
		?end1 prov:hadActivity ?ender1 .
		?end2 prov:hadActivity ?ender2 .
		FILTER (?ender1 != ?ender2)
	}
'''
...
## MA 9/04/15
# Reporting of query failure requires knowing which specific query failed.
# In python there is a one-way mapping from the name of a variable to the value,
#  so if 'merge_start' above is included in a list of queries to run, there's no way to know that 'merge_start' is the
# name of the query that failed
# to solve, we add another layer of indirection

query_map = {'start_precedes_end': start_precedes_end,
			 'start_start_ordering': start_start_ordering,
			 'end_end_ordering': end_end_ordering,
			 'usage_within_activity1': usage_within_activity1,
			 'usage_within_activity2': usage_within_activity2,
			 'generation_within_activity1': generation_within_activity1,
			 'generation_within_activity2': generation_within_activity2,
			 'wasInformedBy_ordering': wasInformedBy_ordering,
			 'generation_precedes_invalidation': generation_precedes_invalidation,
			 'generation_precedes_usage': generation_precedes_usage,
			 'usage_precedes_invalidation': usage_precedes_invalidation,
			 'generation_generation_ordering': generation_generation_ordering,
			 'invalidation_invalidation_ordering': invalidation_invalidation_ordering,
			 'derivation_usage_generation_ordering': derivation_usage_generation_ordering,
			 'derivation_generation_generation_ordering': derivation_generation_generation_ordering,
			 'wasStartedBy_ordering1': wasStartedBy_ordering1,
			 'wasStartedBy_ordering2': wasStartedBy_ordering2,
			 'wasEndedBy_ordering1': wasEndedBy_ordering1,
			 'wasEndedBy_ordering2': wasEndedBy_ordering2,
			 'specialization_generation_ordering': specialization_generation_ordering,
			 'specialization_invalidation_ordering': specialization_invalidation_ordering,
			 'wasAssociatedWith_ordering1': wasAssociatedWith_ordering1,
			 'wasAssociatedWith_ordering2': wasAssociatedWith_ordering2,
			 'wasAssociatedWith_ordering3': wasAssociatedWith_ordering3,
			 'wasAssociatedWith_ordering4': wasAssociatedWith_ordering4,
			 'wasAttributedTo_ordering1': wasAttributedTo_ordering1,
			 'wasAttributedTo_ordering2': wasAttributedTo_ordering2,
			 'actedOnBehalfOf_ordering1': actedOnBehalfOf_ordering1,
			 'actedOnBehalfOf_ordering2': actedOnBehalfOf_ordering2,
			 'unique_generation': unique_generation,
			 'unique_invalidation': unique_invalidation,
			 'unique_wasStartedBy': unique_wasStartedBy,
			 'unique_wasEndedBy': unique_wasEndedBy,
			 'unique_startTime': unique_startTime,
			 'unique_endTime': unique_endTime,
			 'impossible_unspecified_derivation_generation_use': impossible_unspecified_derivation_generation_use,
			 'impossible_specialization_reflexive': impossible_specialization_reflexive,
			 'impossible_property_overlap': impossible_property_overlap,
			 'impossible_object_property_overlap': impossible_object_property_overlap,
			 'entity_activity_disjoint': entity_activity_disjoint,
			 'membership_empty_collection': membership_empty_collection,
			 'key_generation': key_generation,
			 'key_used': key_used,
			 'key_communication': key_communication,
			 'key_start': key_start,
			 'key_end': key_end,
			 'key_invalidation': key_invalidation,
			 'key_derivation': key_derivation,
			 'key_attribution': key_attribution,
			 ...
			 'type_invalidation': type_invalidation,
			 'type_start': type_start,
			 'merge_invalidation': merge_invalidation,
